...
function VortexDrainedByImpact( entity vortexWeapon, entity weapon, entity projectile, damageType )
{
	...
	bool hasVortexRegen = vortexWeapon.HasMod( "tcp_vortex" )	//这边开始检测是否拥有相关mod，以确定启不启用被打中增加使用时间的一堆玩意

	if ( amount <= 0.0 && !hasVortexRegen )		//小于等于？没太拆懂，但还是按照DB佬的代码加上了或
		return
	...
	if( hasVortexRegen )
	{
		...
		/*if ( vortexWeapon.GetWeaponClassName() == "mp_titanweapon_vortex_shield_ion" )
		{
			...
			int energyToAdd = currentEnergy + maxEnergyToAdd >= totalEnergy ? totalEnergy - currentEnergy : maxEnergyToAdd	//嗯，没拆懂，先放这里，拆懂了在加回去
