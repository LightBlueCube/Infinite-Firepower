...
const array<string> docs =    [ "我们的QQ群：150381961",
								...
								"主武器：NULL（空）",
								"核心：GroundSlam[注释10]",
								"[注释10]:VoyageDB大佬制作的核心，对非装甲单位伤害为500，对装甲单位伤害为3500，无衰减伤害范围 0-450  有衰减伤害范围 450-525  超过范围525外则无伤害",
								...
								"中栏位：EMP炸弹[注释12]",
								"[注释12]:表现为强力的声纳突波，效果为电弧场，启动后消耗 当前能量 / 2，落地后，开启0.5秒，伤害类型为EMP手雷，对装甲单位伤害为 25 * 8",
								"对非装甲单位伤害为 1 * 8，当单位进入伤害范围内时，每0.1秒获得一次上述伤害，且离开该范围前将一直拥有被EMP手雷击中的屏幕特效和减速等效果",
								"右栏位：闪光束[注释13]",
								"[注释13]:表现为帝王的满级能量汲取，启动后消耗 当前能量 / 2，冷却时间为 原版冷却时间 * 2，击中敌方泰坦后",
								"敌方泰坦获得一个2秒的致盲 + 正常能量汲取伤害，击中不给使用者加盾，击中队友不给队友加盾",
								"主武器：核裂枪[注释14]",
								...
								"中栏位：电弧场[注释16]",
								"[注释16]:表现为按下后，在泰坦上形成一个电弧场，持续16秒，伤害类型为EMP风暴，对装甲单位伤害为 64，对非装甲单位伤害为 10",
								"当单位进入伤害范围内时，每0.1秒获得一次上述伤害",
								"右栏位：同步彈頭[注释17]",
								...
								"主武器：40cm机炮[注释23]",
								"[注释23]:伤害为 对非装甲单位 200 对装甲单位 900 溅射对非装甲单位 99 溅射对装甲单位 600，无衰减范围 0-100 衰减范围 100-225 范围超过225无伤害",
								...
								"血量：12500（5格）",
								"左栏位：穹顶护盾[注释25]",
								"[注释25]:VoyageDB大佬制作的护盾，血量为3000，持续8秒，冲刺会打破护盾，在使用期间泰坦免疫一切伤害（伤害将转移到护盾），护盾会吃到近战伤害",
								"中栏位：投掷型电子烟幕[注释26]",
								...
								"当射线模式时：发射一道能量汲取，消耗一半的武器充能，击中目标时对目标造成 对装甲单位伤害 200 对非装甲单位伤害 20 并自己回盾2500点",
								"当电球模式时：发射一个速度较慢的电球（速度为200），消耗全部武器充能，在该范围大小为500的范围内会持续收到被电球击中的减益 + EMP减益",
								"和每0.1秒的 对装甲单位伤害 20 对非装甲单位伤害 8，每当且仅当对敌方装甲单位造成一次伤害时都会恢复自己护盾100点，如果护盾已满则恢复50点生命值",
								"当击中目标时对目标造成 对装甲单位伤害 200 对非装甲单位伤害 20",
								"主武器：XO16[注释28]",
								"[注释28]:表现为加装了加速器的远征XO16，最大射速为30，达到最大射速需8秒，伤害为 对非装甲单位 近距离 49 远距离 34  对装甲单位 近距离 100 远距离 80",
								"换弹速度和一代XO16保持一致(4.6s)，弹容60发，瞄准射击扩散不会减小，在核心开启前如果击中涡旋盾则每发会消耗涡旋盾充能的1%，核心开启后不可换弹",
								"弹匣剩余低于等于60即视为核心结束，会重置武器状态致核心前",
								"核心：弹容超载[注释29]",
