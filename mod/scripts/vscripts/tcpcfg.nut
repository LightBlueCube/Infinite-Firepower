...
const array<string> docs =    [ "前言\n因为Emma稳定发挥，broadcast chathook 还有logsystem都是被整寄了\n所以现版本几乎没有什么能发送大量文字的方法\n所以想了这个分页法，所以阅读体验会比较差，因为一次只能显示不几行\n任何更新都会同步更新文档，常来看看！",
								...
								"右栏位：球状闪电[注释28]",
								"[注释28]:表现为帝王的能量汲取，冷却时间为20秒（12 -> 30），拥有两种击发模式，当按住不到1秒就松开时为射线模式，当按住超过1秒时为电球模式",
								"当射线模式时：发射一道能量汲取，消耗一半的武器充能，击中目标时对目标造成 对装甲单位伤害 200 对非装甲单位伤害 20 并自己回盾2500点",
								"当电球模式时：发射一个速度较慢的电球（速度为200），消耗全部武器充能\n在该范围大小为500的范围内会持续收到被电球击中的减益 + EMP减益和每0.1秒的 对装甲单位伤害 20 对非装甲单位伤害 8",
								"每当且仅当对敌方装甲单位造成一次伤害时都会恢复自己护盾100点，如果护盾已满则恢复50点生命值\n当击中目标时对目标造成 对装甲单位伤害 200 对非装甲单位伤害 20",
								"主武器：XO16[注释29]",
								"[注释29]:表现为加装了加速器的远征XO16，最大射速为30，达到最大射速需8秒，伤害为 对非装甲单位 近距离 49 远距离 34  对装甲单位 近距离 100 远距离 80\n换弹速度和一代XO16保持一致(4.6s)，弹容60发，瞄准射击扩散不会减小，在核心开启前如果击中涡旋盾则每发会消耗涡旋盾充能的1%",
								"核心开启后不可换弹，弹匣剩余低于等于60即视为核心结束，会重置武器状态致核心前",
								"核心：弹容超载[注释30]",
								"[注释30]:开启后立刻给主武器增加140发子弹，且仅给主武器增加140发子弹，无任何额外效果，可叠加子弹",
