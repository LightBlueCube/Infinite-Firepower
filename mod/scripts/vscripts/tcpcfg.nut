...
const array<string> docs =    [ "## 特别鸣谢（名字按首字母顺序排列）：",
								"[cmggy][D3-3109][DZ][Pan-da32767][VoyageDB][wolf109909][zxcPandora]",
								"## 无限火力都改了什么？",
								"泰坦获取速度 * 4，泰坦核心获取速度 * 2，泰坦脱战回盾",
								"ai伤害取默认[注释1]，视力取战役ai，瞄准精度为最大，取消开火限制[注释2]",
								"[注释1]:原版ai伤害大概为默认的四分之一",
								"[注释2]:原版ai只能每次打几发子弹点射泼水，现在解除限制后只要ai允许，可以直接打空弹匣",
								...
								"[注释6]:获得一个方向为视角正后方的一个大小为400的没有高度速度的速度 + 一个方向为跳离时按住的方向键的方向的一个大小为200的速度（如果跳离时没按则不给这个速度） + 一个方向为固定朝上的一个大小为400的速度",
								...
								"左栏位：隐身",
								"中栏位：EMP陷阱[注释8]",
								"[注释8]:表现为强力的声纳突波，效果为电弧场，落地后，等待1秒，开启5秒，伤害类型为EMP手雷，对装甲单位伤害为15 * 8，对非装甲单位伤害为1 * 8，当单位进入伤害范围内时，每0.1秒获得一次上述伤害，且离开该范围前将一直拥有被EMP手雷击中的屏幕特效和减速等效果",
								"右栏位：激素（恢复速度 * 2，使用时间 * 2）",
								"近战栏位：melee_titan_punch_fighter[注释9]",
								"[注释9]:废稿武器，伤害改为1750每拳，范围为300，可按左键或F攻击",
								"主武器：NULL（空）",
								"核心：GroundSlam[注释10]",
								"[注释10]:[DB][VoyageDB]大佬制作的核心，对非装甲单位伤害为500，对装甲单位伤害为3500，无衰减伤害范围 0-450  有衰减伤害范围 450-525  超过范围525外则无伤害，启动后立刻恢复激素的冷却，后提供一个根据视角朝向的大小为1200速度 + 高度轴的一个大小为-750的速度，执行烈焰的拍地动作",
								"其他：野牛不会被卡拳，同样的，野牛也无法卡别人拳",
								"### 执政官",
								"启用：离子，携带至尊涂装",
								"血量：10000（4格）",
								"左栏位：自充能涡旋防护罩[注释11]",
								"[注释11]:涡旋盾消耗速度 2.0 -> 4.0，至少需要剩余的能量 >= 100 才可以开启，当武器击中涡旋盾时，增加能量，计算公式为 近距离对装甲单位伤害 / 1250 = 要增加的百分比 如果要增加的百分比 <= 10% 那么用该计算公式: near_val / 625 = 要增加的百分比",
								"中栏位：EMP炸弹[注释12]",
								"[注释12]:表现为强力的声纳突波，效果为电弧场，启动后消耗 当前能量 / 2，落地后，开启0.5秒，伤害类型为EMP手雷，对装甲单位伤害为 25 * 8，对非装甲单位伤害为 1 * 8，当单位进入伤害范围内时，每0.1秒获得一次上述伤害，且离开该范围前将一直拥有被EMP手雷击中的屏幕特效和减速等效果",
								"右栏位：闪光束[注释13]",
								"[注释13]:表现为帝王的满级能量汲取，启动后消耗 当前能量 / 2，冷却时间为 原版冷却时间 * 2，击中敌方泰坦后，敌方泰坦获得一个2秒的致盲 + 正常能量汲取伤害，击中不给使用者加盾，击中队友不给队友加盾",
								"主武器：核裂枪[注释14]",
								"[注释14]:表现为离子的分裂枪，伤害为 原版分裂枪伤害 * 2 （对非装甲单位伤害 近距离 50 远距离 40  对装甲单位伤害 近距离 160 远距离 120），非瞄准射击一发，消耗30能量，瞄准射击三发，消耗90能量，如果能量耗光，非瞄准每射击一次将消耗2发弹药，瞄准每射击一次将消耗6发弹药",
								"核心：雷暴[注释15]",
								...
								"血量：7500（3格）",
								"左栏位：剑封[注释15]",
								"[注释15]:表现为浪人的剑封，减伤50%，有一个充能条，剑封时会消耗，消耗速度为10秒，在消耗殆尽之前会一直拥有1.5倍移速加成，消耗殆尽后移速为 正常移速 * 0.4，冷却2.5秒后才会开始恢复充能，恢复速度为8秒",
								"中栏位：电弧场[注释16]",
								"[注释16]:表现为按下后，在泰坦上形成一个电弧场，持续16秒，伤害类型为EMP风暴，对装甲单位伤害为 64，对非装甲单位伤害为 10，当单位进入伤害范围内时，每0.1秒获得一次上述伤害",
								"右栏位：同步彈頭[注释17]",
								...
								"中栏位：幻影[注释21]",
								"[注释21]:表现为幻影铁驭，使用后表现为生成一个和你一模一样的泰坦幻影，抬头显示的假血量取当前血量，如果黄血则血量取 当前 * 4，冷却时间为20秒，满充能可使用2次，不可以在仅往左移动或仅往右移动时使用",
								"右栏位：绊雷[注释22]",
								"[注释22]:表现为火箭弹群，发射1个黏性绊雷，落地后存在30秒，如果敌方单位靠近则立刻爆炸，伤害为 对非装甲单位 1500 对装甲单位 500，无衰减范围 0-140 衰减范围 140-320 范围超过320无伤害",
								"主武器：40cm机炮[注释23]",
								"[注释23]:伤害为 对非装甲单位 200 对装甲单位 900 溅射对非装甲单位 99 溅射对装甲单位 600，无衰减范围 0-100 衰减范围 100-225 范围超过225无伤害，射速为1.5/s，弹匣容量为10发，核心期间全自动，射速为2/s",
								"核心：破壞核心[注释24]",
								"[注释24]:啟用時大幅提升武器殺傷力。",
								"### 巨妖",
								"启用：军团，携带至尊涂装",
								"血量：12500（5格）",
								"左栏位：穹顶护盾[注释25]",
								"[注释25]:[DB][VoyageDB]大佬制作的护盾，血量为3000，持续8秒，冲刺会打破护盾，在使用期间泰坦免疫一切伤害（伤害将转移到护盾），护盾会吃到近战伤害",
								"中栏位：投掷型电子烟幕[注释26]",
								"[注释26]:表现为强力的声纳突波，在落点生成一个涡旋电子烟幕，对泰坦dps 1000  对铁驭dps 30",
								"右栏位：球状闪电[注释27]",
								"[注释27]:表现为帝王的能量汲取，冷却时间为20秒（12 -> 30），拥有两种击发模式，当按住不到1秒就松开时为射线模式，当按住超过1秒时为电球模式，当射线模式时：发射一道能量汲取，消耗一半的武器充能，击中目标时对目标造成 对装甲单位伤害 200 对非装甲单位伤害 20 并自己回盾2500点。当电球模式时：发射一个速度较慢的电球（速度为200），消耗全部武器充能，在该范围大小为500的范围内会持续收到被电球击中的减益 + EMP减益 和每0.1秒的 对装甲单位伤害 20 对非装甲单位伤害 8，每当且仅当对敌方装甲单位造成一次伤害时都会恢复自己护盾100点，如果护盾已满则恢复50点生命值，当击中目标时对目标造成 对装甲单位伤害 200 对非装甲单位伤害 20",
								"主武器：XO16[注释28]",
								"[注释28]:表现为加装了加速器的远征XO16，最大射速为30，达到最大射速需8秒，伤害为 对非装甲单位 近距离 49 远距离 34  对装甲单位 近距离 100 远距离 80，换弹速度和一代XO16保持一致(4.6s)，弹容60发，瞄准射击扩散不会减小，在核心开启前如果击中涡旋盾则每发会消耗涡旋盾充能的1%，核心开启后不可换弹，弹匣剩余低于等于60即视为核心结束，会重置武器状态致核心前",
								"核心：弹容超载[注释29]",
								"[注释29]:开启后立刻给主武器增加140发子弹，且仅给主武器增加140发子弹，无任何额外效果，可叠加子弹",
								"嘿！如果你看到这句话，你其实可以在控制台里看文档！没必要翻聊天栏" ]
...
void function docsSystem( entity player )
{
	...
	if( !IsValid( player ) )
		return
	Chat_ServerPrivateMessage( player, "嘿！文档需要大约10秒来加载，稍等一下！", false, false )
	wait 3
	foreach( s in docs )
	{
		if( !DontWait )
		...
		if( !IsValid( player ) )
			return
		Chat_ServerPrivateMessage( player, s, false, false )
		if( DontWait )
			DontWait = false
		else
			DontWait = true
