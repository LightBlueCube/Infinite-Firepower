...
void function MpTitanAbilityPowerShot_Init()
{
	...
	RegisterSignal( "PowerShotStopRegen" )
	#endif
}
...
var function OnWeaponPrimaryAttack_power_shot( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( weaponOwner.IsPlayer() )
	{
		...
		thread StopWeaponRegen( weaponOwner, weapon )
		#endif
	}
...
void function StopWeaponRegen( entity player, entity weapon )
{
	...
	player.EndSignal( "PowerShotCleanup" )

	player.Signal( "PowerShotStopRegen" )
	player.EndSignal( "PowerShotStopRegen" )

	WaitFrame()
	int clip = weapon.GetWeaponPrimaryClipCount()
	for( ;; )
	{
		if( weapon.GetWeaponPrimaryClipCount() < clip )
			clip = weapon.GetWeaponPrimaryClipCount()
		weapon.SetWeaponPrimaryClipCountAbsolute( clip )
		WaitFrame()
	}
}
#endif
