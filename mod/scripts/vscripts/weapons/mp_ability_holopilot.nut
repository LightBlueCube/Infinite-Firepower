...
var function OnWeaponPrimaryAttack_holopilot( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if( weapon.HasMod( "tcp_titan_holo" ) && XOR( weaponOwner.IsInputCommandHeld( IN_MOVELEFT ), weaponOwner.IsInputCommandHeld( IN_MOVERIGHT ) ) && !( weaponOwner.IsInputCommandHeld( IN_FORWARD ) || weaponOwner.IsInputCommandHeld( IN_BACK ) ) )
		return 0


#if SERVER
	if( weapon.HasMod( "tcp_titan_holo" ) )
		CreateHoloPilotDecoys( weaponOwner, 1, true )
	else
...
bool function XOR( bool a, bool b )
{
	if( ( a && b ) || ( !a && !b ) )
		return false
	if( ( a && !b ) || ( !a && b ) )
...
void function CreateHoloPilotDecoys( entity player, int numberOfDecoysToMake = 1, bool titanHolo = false )
{
	...
	for( int i = 0; i < numberOfDecoysToMake; ++i )
	{
		...
		if( titanHolo && player.IsTitan() )
		{
			...
			if( IsValid( soul ) )
			{
				if( soul.IsDoomed() )
				{
					decoy.SetMaxHealth( player.GetMaxHealth() * 4 )
					decoy.SetHealth( player.GetHealth() * 4 )
				}
			}
		}
